...
export update_very_day_jieguo=`cat ${path}/${log_name} | grep "update_OK" | wc -l`

if [ "${update_very_day_jieguo}" -eq 1 ]
then
  export msg_body_info='<font color=\"info\">代报数据库后台自动更新成功 @台建菊!</font>'
else
  export msg_body_info='`更新失败 @台建菊!,请联系技术支持`'
fi
